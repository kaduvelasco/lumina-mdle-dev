...
`lumina-mdle-dev` exposes an **MCP prompt** called `scaffold_plugin` that automatically injects the full Moodle context — version, coding standards, and naming conventions — into the scaffold generation. Invoke it in the AI chat:
...
- [ ] Ask your assistant: _"Run the lumina-mdle-dev doctor"_ — the AI will call the `doctor` tool and confirm if the new plugin was indexed correctly.
