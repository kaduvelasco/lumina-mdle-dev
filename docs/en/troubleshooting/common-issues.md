...
Running into issues with `lumina-mdle-dev`? This page covers the most common errors with direct solutions.
...
If `lumina-mdle-dev` appears as connected with 11 tools, the server is working. The problem is in the context configuration, not the connection.
...
**Solution:**
```
Reinitialize the lumina-mdle-dev context for the Moodle installation.
```
...
### The AI says it doesn't know `lumina-mdle-dev`

**Symptom:** The AI responds that it doesn't have access to the server or doesn't know what `lumina-mdle-dev` is.
