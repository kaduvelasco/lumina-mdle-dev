...
`lumina-mdle-dev` was designed to be flexible in containerized environments. If you use **LuminaStack** or any Docker stack (Nginx + PHP-FPM + MariaDB), there are two main deployment scenarios.
...
`lumina-mdle-dev` runs on the host and reads these files directly as a normal local directory.
...
```
Initialize the lumina-mdle-dev context for my Moodle installation.
```
...
Since `lumina-mdle-dev` also runs as the host user, writing `.md` context files works without additional configuration.
