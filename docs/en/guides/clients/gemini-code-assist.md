...
You will see `lumina-mdle-dev` listed as connected with all 11 tools available. If it does not appear, see the [Troubleshooting](#️-troubleshooting) section.
...
## lumina-mdle-dev
The lumina-mdle-dev MCP server is connected in Agent Mode.
...
Run `/mcp` in the Agent Mode chat. If `lumina-mdle-dev` does not appear, the problem is in the configuration — not your prompt.
