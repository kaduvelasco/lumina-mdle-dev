...
## lumina-mdle-dev
The lumina-mdle-dev MCP server is configured.
...
```
Initialize the lumina-mdle-dev context for this Moodle installation.
```
...
- Arrays use brackets: `args = ["-y", "lumina-mdle-dev"]`
- Section name must be exact: `[mcp_servers.lumina-mdle-dev]`

### SSE is not supported

Codex only supports **stdio** for local servers. `lumina-mdle-dev` uses stdio by default — no additional configuration needed. The server's HTTP mode (`--http`) is not compatible with Codex for local use.
...
- [Antigravity CLI](./antigravity.md) — Google's terminal agent
