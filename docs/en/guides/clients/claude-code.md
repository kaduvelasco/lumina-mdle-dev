...
You will see `lumina-mdle-dev` listed as connected with all 11 tools available. If the server does not appear, see the [Troubleshooting](#️-troubleshooting) section.
...
## lumina-mdle-dev
The lumina-mdle-dev MCP server is connected and indexes have been generated.
...
Before any other investigation, run `/mcp` inside the Claude session. If `lumina-mdle-dev` does not appear as connected, the problem is in the server configuration, not your prompt.
...
```
Run the lumina-mdle-dev doctor.
```
