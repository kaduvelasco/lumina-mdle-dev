...
`lumina-mdle-dev` uses a cache system based on **mtime** (file modification time) to avoid regenerating context files unnecessarily. Without caching, every call to `update_indexes` or `generate_plugin_context` would reprocess all PHP and XML files in the installation — regardless of whether they changed or not.
