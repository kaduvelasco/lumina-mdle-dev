...
All 12 MCP tools exposed by lumina-mdle-dev.
...
| `mark_as_dev` | boolean | `false` | Mark processed plugins with `.indevelopment` |
...
Lists all plugins marked as in development — i.e., those that have a `.indevelopment` file in their directory.
...
The server identifies which plugins you are developing by the presence of a `.indevelopment` file in the plugin directory. This marker is used by `list_dev_plugins`, `watch_plugins`, and `plugin_batch mode="dev"`.
...
```bash
touch /your/moodle/local/myplugin/.indevelopment
```
...
```bash
rm /your/moodle/local/myplugin/.indevelopment
```
...
The assistant will call `list_dev_plugins` and list all directories containing `.indevelopment`.
...
| File | Reason |
|------|--------|
| `PLUGIN_AI_CONTEXT.md`, `PLUGIN_ARCHITECTURE.md`, `PLUGIN_CALLBACK_INDEX.md`, `PLUGIN_CONTEXT.md`, `PLUGIN_DB_TABLES.md`, `PLUGIN_DEPENDENCIES.md`, `PLUGIN_ENDPOINT_INDEX.md`, `PLUGIN_EVENTS.md`, `PLUGIN_FUNCTION_INDEX.md`, `PLUGIN_RUNTIME_FLOW.md`, `PLUGIN_STRUCTURE.md` | Generated by lumina-mdle-dev |
