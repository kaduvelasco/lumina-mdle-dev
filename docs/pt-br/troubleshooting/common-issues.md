...
Encontrou algum problema ao usar o `lumina-mdle-dev`? Esta página cobre os erros mais comuns com soluções diretas.
...
Se `lumina-mdle-dev` aparecer como conectado com 11 tools, o servidor está funcionando. O problema está na configuração do contexto, não na conexão.
...
**Solução:**
```
Reinicialize o contexto do lumina-mdle-dev para a instalação Moodle.
```
...
### A IA diz que não conhece o `lumina-mdle-dev`

**Sintoma:** A IA responde que não tem acesso ao servidor ou não sabe o que é o `lumina-mdle-dev`.
