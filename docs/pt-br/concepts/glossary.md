...
Termos técnicos usados na documentação do `lumina-mdle-dev`, organizados alfabeticamente.
...
Módulo interno do `lumina-mdle-dev` responsável por ler e fazer parse de um tipo específico de arquivo PHP do Moodle. Cada extractor produz dados estruturados que são consumidos pelos Generators.

Exemplos: `schema.ts` lê `db/install.xml`; `events.ts` lê `db/events.php`; `api.ts` lê `lib/*.php`.

→ Veja: [Extractors](../architecture/extractors.md) · [Como o servidor funciona](./how-lumina-mdle-dev-works.md)
...
Módulo interno do `lumina-mdle-dev` responsável por transformar os dados produzidos pelos Extractors em arquivos `.md` de contexto escritos diretamente na instalação Moodle. Existem dois tipos: generators globais (escrevem na raiz do Moodle) e generators de plugin (escrevem dentro de cada diretório de plugin).
...
O `lumina-mdle-dev` detecta e indexa tanto os callbacks legados quanto as definições da Hook API em `db/hooks.php` e `classes/hook/`.
...
O `lumina-mdle-dev` é um servidor MCP especializado em desenvolvimento de plugins Moodle.
...
O `lumina-mdle-dev` expõe três prompts:
...
O `lumina-mdle-dev` expõe 11 tools:
