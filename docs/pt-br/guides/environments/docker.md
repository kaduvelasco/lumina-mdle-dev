...
O `lumina-mdle-dev` foi projetado para ser flexível em ambientes containerizados. Se você utiliza o **LuminaStack** ou qualquer stack Docker (Nginx + PHP-FPM + MariaDB), existem dois cenários principais de operação.
...
No LuminaStack, o diretório Moodle fica em `~/workspace/www/html/<projeto>/` — montado do host para dentro dos containers PHP. O `lumina-mdle-dev` roda no host e lê esses arquivos diretamente, como se fossem um diretório local qualquer.
...
**Configuração do cliente de IA (Antigravity CLI — `~/.gemini/antigravity/mcp_config.json`):**
...
```
Inicialize o contexto do lumina-mdle-dev para minha instalação Moodle.
```
...
services:
    lumina-mdle-dev:
        image: node:18-slim
        container_name: lumina-mdle-dev
        working_dir: /app
        volumes:
            - ./lumina-mdle-dev:/app # código do servidor MCP
...
No LuminaStack, os arquivos em `~/workspace/www/html/moodle/` pertencem ao usuário do host. O `lumina-mdle-dev` roda no host com esse mesmo usuário, então a escrita de arquivos `.md` de contexto funciona sem configuração adicional.
...
docker logs -f lumina-mdle-dev

# Reiniciar após atualizar o código do MCP
docker compose restart lumina-mdle-dev
```
