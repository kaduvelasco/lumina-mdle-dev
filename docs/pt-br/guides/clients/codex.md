...
## lumina-mdle-dev
O servidor MCP lumina-mdle-dev está configurado.
...
```
Inicialize o contexto do lumina-mdle-dev para esta instalação Moodle.
```
...
- Arrays usam colchetes: `args = ["-y", "lumina-mdle-dev"]`
- O nome da seção deve ser exato: `[mcp_servers.lumina-mdle-dev]`
...
O Codex suporta apenas **stdio** para servidores locais. O `lumina-mdle-dev` usa stdio por padrão — sem configuração adicional necessária. O modo HTTP (`--http`) do servidor não é compatível com o Codex para uso local.
