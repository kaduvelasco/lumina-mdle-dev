...
Você verá `lumina-mdle-dev` listado como conectado com as 11 tools disponíveis. Se não aparecer, veja a seção de [Troubleshooting](#️-solução-de-problemas).
...
## lumina-mdle-dev
O servidor MCP lumina-mdle-dev está conectado no modo Agent.
...
Execute `/mcp` no chat do Agent Mode. Se `lumina-mdle-dev` não aparecer, o problema está na configuração — não no seu prompt.
