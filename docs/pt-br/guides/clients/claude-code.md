...
Você verá `lumina-mdle-dev` listado como conectado, com as 11 tools disponíveis. Se o servidor não aparecer, veja a seção de [Troubleshooting](#️-troubleshooting).
...
## lumina-mdle-dev
O servidor MCP lumina-mdle-dev está conectado e os índices foram gerados.
...
Antes de qualquer outra investigação, execute `/mcp` dentro da sessão do Claude. Se `lumina-mdle-dev` não aparecer como conectado, o problema está na configuração do servidor, não no seu prompt.
...
claude mcp remove lumina-mdle-dev
claude mcp add lumina-mdle-dev \
  -e MOODLE_PATH=/home/usuario/workspace/www/html/moodle \
...
```
Execute o doctor do lumina-mdle-dev.
```
...
- [Antigravity CLI](./antigravity.md) — guia equivalente para o agente de terminal do Google
