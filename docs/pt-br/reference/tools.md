...
- _"Inicialize o contexto do lumina-mdle-dev para minha instalação Moodle."_
...
**Exemplo:**
```
Inicialize o contexto do lumina-mdle-dev para a instalação em
...
|-----------|------|:-----------:|--------|-----------|
| `mode` | string | ✅ | `dev` | `dev` — plugins com `.indevelopment`; `all` — todos os plugins; `list` — lista específica |
...
| `mark_as_dev` | boolean | ❌ | `false` | Se `true`, cria o arquivo `.indevelopment` nos plugins processados |
...
Lista todos os plugins marcados como em desenvolvimento na instalação — ou seja, que possuem o arquivo `.indevelopment` em seu diretório.
...
O servidor identifica quais plugins você está desenvolvendo pela presença do arquivo `.indevelopment` no diretório do plugin. Esse marcador é usado por `list_dev_plugins`, `watch_plugins` e `plugin_batch mode="dev"`.
...
```bash
touch /seu/moodle/local/myplugin/.indevelopment
```
...
```bash
rm /seu/moodle/local/myplugin/.indevelopment
```
...
O assistente chamará `list_dev_plugins` e listará todos os diretórios com `.indevelopment`.
...
Diagnostica o ambiente do `lumina-mdle-dev` e gera um relatório de saúde.
...
> `doctor` é uma **tool MCP** — não um comando CLI. Execute pedindo ao assistente: _"Execute o doctor do lumina-mdle-dev."_

**Exemplo:**
```
Execute o doctor do lumina-mdle-dev e me diga se há algum problema
...
| Arquivo | Motivo |
|---------|--------|
| `PLUGIN_AI_CONTEXT.md`, `PLUGIN_ARCHITECTURE.md`, `PLUGIN_CALLBACK_INDEX.md`, `PLUGIN_CONTEXT.md`, `PLUGIN_DB_TABLES.md`, `PLUGIN_DEPENDENCIES.md`, `PLUGIN_ENDPOINT_INDEX.md`, `PLUGIN_EVENTS.md`, `PLUGIN_FUNCTION_INDEX.md`, `PLUGIN_RUNTIME_FLOW.md`, `PLUGIN_STRUCTURE.md` | Gerados pelo lumina-mdle-dev |
