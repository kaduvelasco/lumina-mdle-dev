...
O `lumina-mdle-dev` usa um sistema de cache baseado em **mtime** (data de modificação de arquivo) para evitar regenerar arquivos de contexto desnecessariamente. Sem cache, cada chamada a `update_indexes` ou `generate_plugin_context` reprocessaria todos os arquivos PHP e XML da instalação — independentemente de terem mudado ou não.
