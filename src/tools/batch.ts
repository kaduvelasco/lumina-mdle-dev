 ...
 *
 *   dev      (default) — all plugins marked with .indevelopment
 ...
 * Covers all plugin types at their correct filesystem depths.
 */
async function findAllPlugins(moodlePath: string): Promise<string[]> {
  const patterns = Object.values(PLUGIN_TYPE_TO_DIR).map((d) => `${d}/*/version.php`);
...
export function registerBatchTool(server: McpServer): void {
  server.tool(
    ...
    "Three modes: " +
    "'dev' (default) processes all plugins marked with .indevelopment; " +
...

    {
      mode: z
        ...
        .default("dev")
        .describe(
          "'dev' — only plugins marked with .indevelopment (default). " +
      ...
      mark_as_dev: z
        ...
        .describe(
          ...
          ".indevelopment so they are picked up by watch_plugins and future " +
    ...
    async ({ mode, plugins: pluginList, force, mark_as_dev }) => {
      ...
      if (mode === "dev") {
        ...
        modeDescription = "dev plugins (.indevelopment)";
      ...
      if ((mode === "all" || mode === "list") && succeeded.length > 0) {
        if (mark_as_dev) {
          lines.push(
            "",
            `Marked ${succeeded.length} plugin(s) with .indevelopment for automatic watch ` +
