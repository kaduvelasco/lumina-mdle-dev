...
/**
 * Checks if a command is available on the system PATH without blocking the event loop.
 * Uses `which` (Unix) or `where` (Windows) — both are real executables, not shell builtins.
 */
async function commandExists(cmd: string): Promise<boolean> {
  try {
    const checker = process.platform === "win32" ? "where" : "which";
    await execFileAsync(checker, [cmd]);
    return true;
  } catch {
    return false;
  }
...
export function registerDoctorTool(server: McpServer): void {
  server.tool(
    ...
    async () => {
      ...
      const depChecks: CheckResult[] = await Promise.all(
        deps.map(async ({ cmd, label, required }) => {
