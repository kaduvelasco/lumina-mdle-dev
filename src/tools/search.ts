 ...
 * File content is cached in memory and invalidated when the file mtime changes.
 */
function searchInFile(filePath: string, query: string): string[] {
  ...
  return cached.lines.filter((line) => line.toLowerCase().includes(lowerQ));
}
...
export function registerSearchTools(server: McpServer): void {
  ...
  server.tool(
    ...
    "(those containing a .indevelopment file). " +
    "Requires init_moodle_context to have been run first.",
