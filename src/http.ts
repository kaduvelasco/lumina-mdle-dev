...
function makeLegacySseHandler(
  ...
  return async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      // After connect, sessionId must be set by the transport
      const sessionId = transport.sessionId;
      if (!sessionId) {
        throw new Error("SSEServerTransport did not assign a sessionId after connect");
      }
...
export async function startHttpServer(options: HttpServerOptions): Promise<() => Promise<void>> {
  ...
  // Health check registered before auth — load balancers and monitors need unauthenticated access
