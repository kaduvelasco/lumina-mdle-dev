...
export class MtimeCache {
  ...
  isStale(outputFile: string, sourceFiles: string[]): boolean {
    ...
    // Already marked fresh this session — but check whether any source changed since then
    if (this.marked.has(outputFile)) {
      const markedAt = this.marked.get(outputFile)!;
      const changedAfterMark = sourceFiles.some(
        (src) => existsSync(src) && this.getMtime(src) > markedAt
      );
      if (!changedAfterMark) {
        this.stats.hits++;
        return false;
      }
      // Source changed after mark — invalidate and fall through to regenerate
