...
Use **verb_noun** format, lowercase, underscore-separated. This project uses `snake_case` rather than `kebab-case` because Moodle's own naming conventions are snake_case and AI clients handle both equally well.
...
Name must still follow the verb-noun semantic structure: the first segment is an action verb, the second is the subject.
